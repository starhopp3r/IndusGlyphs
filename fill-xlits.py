...
df = pd.read_csv('dataset/xlits.csv', dtype={'sign': str, 'canonical': str})
...
def compute_xlit(row):
    ...
    # Concatenate the xlits and reverse (R-L)
    new_xlit = ''.join(xlit_components)[::-1]
    return new_xlit
...
df.to_csv('dataset/xlits.csv', index=False)
